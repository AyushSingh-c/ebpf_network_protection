...
namespace pcpp
{
	...
	std::string EthLayer::toTSVString() const
	{
		return "Ethernet II Layer\t" + getSourceMac().toString() + "\t" + getDestMac().toString();
	}

	std::string EthLayer::toJSONString() const
	{
		return "{\n\"Layer\": \"Ethernet II Layer\",\n\"Src\": \"" + getSourceMac().toString() + "\",\n\"Dst\": \"" + getDestMac().toString() + "\"\n}";
	}
