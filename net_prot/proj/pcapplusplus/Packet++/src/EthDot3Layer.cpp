...
namespace pcpp
{
	...
	// |	Layer	|	Src MAC	|	Dst Mac	|
	std::string EthDot3Layer::toTSVString() const
	{
		return "IEEE 802.3 Ethernet\t" + getSourceMac().toString() + "\t" + getDestMac().toString();
	}

	std::string EthDot3Layer::toJSONString() const
	{
		return "{\n\"Layer\": \"IEEE 802.3 Ethernet\",\n\"Src\": \"" + getSourceMac().toString() + "\",\n\"Dst\": \"" + getDestMac().toString() + "\"\n}";
	}
