...
#define DHCP_MAGIC_NUMBER 0x63538263
	...
	std::string DhcpLayer::getMessageTypeAsString() const
	{
	...
	{
		std::string msgType = getMessageTypeAsString();
		return "DHCP layer (" + msgType + ")";
	}

	// |	Layer	|	message type	|
	std::string DhcpLayer::toTSVString() const
	{
		std::string msgType = getMessageTypeAsString();
		return "DHCP layer\t" + msgType;
	}

	std::string DhcpLayer::toJSONString() const
	{
		std::string msgType = getMessageTypeAsString();
