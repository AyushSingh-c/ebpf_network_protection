...
namespace pcpp
{
	...
	std::string DnsLayer::toString() const
	{
		...
		DnsResource* curAnswer = getFirstAnswer();
		for (size_t i = 0; i < getAnswerCount(); i++)
		{
			if (curAnswer != nullptr && curAnswer->getData() != nullptr)
				answers << curAnswer->getData()->toString();
			curAnswer = getNextAnswer(curAnswer);
			if (curAnswer != nullptr)
				answers << ", ";
		}
		std::ostringstream queries;
		DnsQuery* curQuery = getFirstQuery();
		for (size_t i = 0; i < getQueryCount(); i++)
		{
			if (curQuery != nullptr && curQuery != nullptr)
				queries << curQuery->getName();
			curQuery = getNextQuery(curQuery);
			if (curQuery != nullptr)
				queries << ", ";
		}
		...
		if (getDnsHeader()->queryOrResponse == 1)
		{
			return "DNS query response, ID: " + tidAsString.str() + ";" + " queries: " + queryCount.str() +
				   " queries data: " + queries.str() + ", answers: " + answerCount.str() + " answers data: " + answers.str() +
	...
	// |	Layer	|	Message Type	|	ID	|	Queries Count	|	Queries   |	Answers Count	|	Answers	|	Authorities	|	Additional	|
	std::string DnsLayer::toTSVString() const
	{
		std::ostringstream tidAsString;
		tidAsString << be16toh(getDnsHeader()->transactionID);
		...
		if (getDnsHeader()->queryOrResponse == 1)
		{
			return "DNS Layer\tquery response\t" + tidAsString.str() + "\t" + queryCount.str() +
				   "\t" + queries.str() + "\t" + answerCount.str() + "\t" + answers.str() +
				   "\t" + authorityCount.str() + "\t" + additionalCount.str();
		}
		else if (getDnsHeader()->queryOrResponse == 0)
		{
			return "DNS Layer\tquery\t" + tidAsString.str() + "\t" + queryCount.str() +
				   "\t" + queries.str() + "\t" + answerCount.str() + "\t" + answers.str() +
				   "\t" + authorityCount.str() + "\t" + additionalCount.str();
		}
		else  // not likely - a DNS with no answers and no queries
		{
			return "DNS Layer\trecord without queries and answers\t" + tidAsString.str() + "\t" +
			    	queryCount.str() + "\t-\t" + answerCount.str() +
			       "\t-\t" + authorityCount.str() + "\t" + additionalCount.str();
		}
	}

	std::string DnsLayer::toJSONString() const
	{
		std::ostringstream tidAsString;
		tidAsString << be16toh(getDnsHeader()->transactionID);
