...
namespace pcpp
{
	...
	std::string Packet::toTSVString(bool timeAsLocalTime) const
	{
		std::vector<std::string> stringList;
		toTSVStringList(stringList, timeAsLocalTime);
		return std::accumulate(stringList.begin(), stringList.end(), std::string(),
		                       [](std::string a, const std::string& b) { return std::move(a) + b + '\n'; });
	}

	void Packet::toTSVStringList(std::vector<std::string>& result, bool timeAsLocalTime) const
	{
		result.clear();
		result.push_back(printPacketInfo(timeAsLocalTime));
		Layer* curLayer = m_FirstLayer;
		while (curLayer != nullptr)
		{
			result.push_back(curLayer->toTSVString());
			curLayer = curLayer->getNextLayer();
		}
	}

	std::string Packet::toJSONString(bool timeAsLocalTime) const
	{
		std::vector<std::string> stringList;
		toJSONStringList(stringList, timeAsLocalTime);
		return std::accumulate(stringList.begin(), stringList.end(), std::string(),
		                       [](std::string a, const std::string& b) { return std::move(a) + b + '\n'; });
	}

	void Packet::toJSONStringList(std::vector<std::string>& result, bool timeAsLocalTime) const
	{
		result.clear();
		result.push_back(printPacketInfo(timeAsLocalTime));
		Layer* curLayer = m_FirstLayer;
		while (curLayer != nullptr)
		{
			result.push_back(curLayer->toJSONString());
			curLayer = curLayer->getNextLayer();
		}
	}
